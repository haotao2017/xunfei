...
const APPID = 'c4661e2a'
const API_SECRET = 'ZTYxNWE3Mzc4Njk2YzM5MmVhNmExMjc1'
const API_KEY = 'c160ed57c4bdeffdd1fe1c8ef6d10b7c'
...
iseRecorder.onWillStatusChange = function(oldStatus, status) {
  ...
  if (status === 'ing') {
    ...
    countInterval = setInterval(()=>{
      ...
      time = '0' + Math.floor(senconds / 60) + ': ' + second
      $('.used-time').text(time)
      if (senconds >= 180) {
        this.stop()
...
iseRecorder.onTextChange = function(grade) {
  ...
  $('.accuracy_score').text(readSentence?.accuracy_score)
  $('.emotion_score').text(readSentence?.emotion_score)
  $('.fluency_score').text(readSentence?.fluency_score)
  $('.integrity_score').text(readSentence?.integrity_score)
  $('.phone_score').text(readSentence?.phone_score)
  $('.tone_score').text(readSentence?.tone_score)
  $('.total_score').text(readSentence?.total_score)
  let sentence = readSentence?.sentence?.word || readSentence?.sentence || []
  let resultStr = ''
...
$('#taste_button, .taste-button').click(() => {
